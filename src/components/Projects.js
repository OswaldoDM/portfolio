            ...
            <div className='grid justify-between grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-x-10 gap-14 lg:gap-x-14 lg:gap-y-20 2xl:gap-x-10'>
               {projects.map(project => (
                  <div key={project.label} className='flex flex-col items-center sm:items-start'>
                     ...
                     {project.icon4 &&
                     <div className="flex gap-1 justify-center sm:justify-start items-center w-full font-mono mt-3">
                        <span className={`${project.icon4Color} ${project.icon4Size} flex items-center`}>
                           {project.icon4}
                        </span>
                        <p className='text-[11px] mr-2'>{project.icon4Label}</p>
                        {project.icon5 &&
                           <>
                              <span className={`${project.icon5Color} ${project.icon5Size} flex items-center`}>
                                 {project.icon5}
                              </span>
                              <p className='text-[11px] mr-2'>{project.icon5Label}</p>
                           </>
                        }
                        {project.icon6 &&
                           <>
                              <span className={`${project.icon6Color} ${project.icon6Size} flex items-center`}>
                                 {project.icon6}
                              </span>
                              <p className='text-[11px] mr-2'>{project.icon6Label}</p>
                           </>
                        }
                     </div>
                     }                     
